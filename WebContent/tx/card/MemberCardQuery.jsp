...
<body>
 <div class="bs-example bs-example-form" >
	 <p class="bg-success" style="text-align: center;padding: 5px;background-color: #EEEEEE;border-radius: 3px;border: 1px solid #CCCCCC;">开卡信息查询</p>
	 ...
	 <div class="input-group">
      <div style="display: inline-block; width:33%;padding: 2px;font-size: 14px;" >
        <input type="radio" name="DateType"  id ="DateType1" checked value='1'>
        <label for="DateType1" class="btn btn-default" >充值日期</label>
       </div>   
      <div style="display: inline-block; width:33%;padding: 2px;font-size: 14px;" >
        <input type="radio" name="DateType"  id ="DateType2"  value='2'>
        <label for="DateType2" class="btn btn-default" >消费日期</label>
       </div>   
      <div style="display: inline-block; width:33%;padding: 2px;font-size: 14px;" >
        <input type="radio" name="DateType"  id ="DateType3"  value='3'>
        <label for="DateType3" class="btn btn-default" >有效日期</label>
       </div>   
   </div>
...
<script type="text/javascript"> 
 ...
 function Query()
 {
	   ...
	   var DateType=$("[type=radio]:checked").attr("value");
	   //return;
   
	 $("#loadingToast").show();
   $.post("<%=request.getContextPath() %>/card/query/MemberCardQuery",{"GuestTel":GuestTel,"GuestName":GuestName,"BeginDate":BeginDate,"EndDate":EndDate,"DateType":DateType},function(data){
         //console.log(data);
         ...
         else
         {
            ...
            if (data.Data.length==0)
            {
             ...
             var th=$("<thead><td>姓名</td><td>手机</td><td>开卡日期</td><td>有效期</td><td>剩余次数</td></thead>")
             table.append(th);
             ...
             for (var i=0;i<data.Data.length;i++)
             {
               ...
               var tr=$("<tr><td><a href=\"<%=request.getContextPath()%>/tx/card/MemberCardEdit.jsp?ID="+data.Data[i].ID+"\">"+data.Data[i].Name+"</a></td>"+
            		    "<td><a href=\"<%=request.getContextPath()%>/tx/card/MemberCardEdit.jsp?ID="+data.Data[i].ID+"\">"+data.Data[i].Tel+"</a></td>"+
            		    "<td><a href=\"<%=request.getContextPath()%>/tx/card/MemberCardEdit.jsp?ID="+data.Data[i].ID+"\">"+data.Data[i].CreateDate+"</a></td>"+
            		    "<td><a href=\"<%=request.getContextPath()%>/tx/card/MemberCardEdit.jsp?ID="+data.Data[i].ID+"\">"+data.Data[i].EndDate+"</a></td>"+
            		    "<td><a href=\"<%=request.getContextPath()%>/tx/card/MemberCardEdit.jsp?ID="+data.Data[i].ID+"\">"+data.Data[i].Count+"</a></td></tr>");
               body.append(tr);
             }
             table.append(body);
